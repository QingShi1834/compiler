...
//        localScopeNum++;
    ...
    public Void visitVarDecl(SysYParser.VarDeclContext ctx) {
        ...
        for (SysYParser.VarDefContext varDefContext : ctx.varDef()) {
            // 报告 Error type 3
//            Type varType = (Type) globalScope.findSymbol(typeName);
...
//            Type constType = (Type) globalScope.findSymbol(typeName);

            String constVarName = varDefContext.IDENT().getText();
            if (currentScopePointer.definedSymbol(constVarName)){
...
//        Type varType = (Type) globalScope.findSymbol(varTypeName);
    ...
    public Void visitStmt(SysYParser.StmtContext ctx) {
        if (ctx.ASSIGN() != null) {
            ...
            } else if (!leftType.toString().equals("noType") && !rightType.toString().equals("noType") && !leftType.toString().equals(rightType.toString())) {
                // 报告 Error type 5
